...
import { BADGE_TOOLTIPS, IVDisplay, PeopleSection, ElitaBadge } from '../components/PeopleSection';
...
const CSS = `
  ...
  .cd-hero-img { overflow:hidden; }
  .cd-hero-img img { transition:transform .7s cubic-bezier(.22,1,.36,1); transform-origin:center; will-change:transform; }
  .cd-hero-banner:hover .cd-hero-img img { transform:scale(1.06); }

  .cd-section { position:relative; z-index:1; opacity:0; transform:translateY(20px); transition:opacity .55s cubic-bezier(.4,0,.2,1), transform .55s cubic-bezier(.4,0,.2,1); }
  .cd-section.vis { opacity:1; transform:translateY(0); }
  .cd-section:has(.c-mil-shell:hover),
  .cd-section:has(.course-card-shell:hover),
  .cd-section:has(.prof-card-shell:hover) { z-index:80; }

  .cd-lesson-shell.locked:hover .c-lock-icon { animation:lockShake .4s ease; }
  ...
  .cd-series-shell.locked:hover .c-lock-icon { animation:lockShake .4s ease; }
  ...
  .cd-person-card:hover { box-shadow:0 22px 60px rgba(55,93,251,.16) !important; border-color:#B8CAFE !important; }
  ...
  .cd-meta-card:hover { box-shadow:0 8px 28px rgba(55,93,251,.1); }

  .cd-review-card { animation:reviewIn .3s ease backwards; transition:box-shadow .2s, transform .2s; }
  .cd-review-card:hover { box-shadow:0 6px 24px rgba(55,93,251,.1); }
  ...
  /* Анимация «военное дело» для карточек занятий и курсов серии */
  .cd-lesson-shell .c-card-img { height:200px; }
  .cd-lesson-shell .c-card-wrap { border-radius:18px; }
  .cd-lesson-grid { position:relative; overflow:visible; }
  .cd-viewed-badge { width:30px;padding:0!important;justify-content:center;overflow:hidden;transition:width .34s cubic-bezier(.22,1,.36,1),padding .34s cubic-bezier(.22,1,.36,1),gap .34s ease; }
  .cd-viewed-label { width:0;opacity:0;transform:translateX(-4px);white-space:nowrap;overflow:hidden;transition:width .34s cubic-bezier(.22,1,.36,1),opacity .2s .06s ease,transform .34s cubic-bezier(.22,1,.36,1); }
  .cd-lesson-shell:hover .cd-viewed-badge { width:112px;padding:0 10px!important; }
  .cd-lesson-shell:hover .cd-viewed-label { width:72px;opacity:1;transform:translateX(0); }
  .cd-series-shell .c-card-img { height:210px; }
  .cd-series-shell .c-card-wrap { border-radius:18px; }
  ${COURSE_CARD_MOTION_CSS}

  /* Карточки и их секции остаются на месте; движение сохраняется только у изображений. */
  .cd-lesson-shell .c-expand-wrap,
  .cd-series-shell .c-expand-wrap,
  .cd-lesson-shell .c-oi,
  .cd-series-shell .c-oi { transform:none !important; }
`;
...
function BadgeBox({ src, tooltip, size = 64, topRight }: { src: string; tooltip?: string; size?: number; topRight?: React.ReactNode }) {
  const [err, setErr] = useState(false);
  const [show, setShow] = useState(false);
  return (
    <div className="cd-badge-box" style={{ width: size, height: size, flexShrink: 0, background: show ? '#EAF0FF' : '#F6F8FA', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 6, border: `1.5px solid ${show ? '#8EA8FF' : '#EAECF0'}`, cursor: tooltip ? 'pointer' : 'default', position: 'relative', animation: 'vBadgeIn .4s ease backwards', boxShadow:show?'0 8px 22px rgba(35,73,180,.18)':'none', transform:show?'translateY(-1px) scale(1.04)':'none', transition:'all .22s ease' }}
      ...
      {topRight && <div style={{ position: 'absolute', top: size <= 48 ? -7 : -8, right: size <= 48 ? -7 : -8, zIndex: 10, pointerEvents: 'none' }}>{topRight}</div>}
      {tooltip && show && (
        <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: '50%', transform: 'translateX(-50%)', background: '#122A66', border:'1px solid #5578DE', color: '#fff', fontSize: 12, fontWeight:700, padding: '8px 13px', borderRadius: 10, whiteSpace: 'nowrap', zIndex: 500, pointerEvents: 'none', boxShadow:'0 10px 28px rgba(18,42,102,.28)' }}>
          {tooltip}
          <div style={{ position: 'absolute', bottom: -4, left: '50%', transform: 'translateX(-50%) rotate(45deg)', width: 7, height: 7, background: '#122A66', borderRight:'1px solid #5578DE', borderBottom:'1px solid #5578DE' }} />
        </div>
      )}
    </div>
  );
    ...
    <div className="cd-modal-overlay" onClick={onClose}>
      <div className="cd-modal-box" onClick={e => e.stopPropagation()} style={{ width: 'min(440px, 100%)', overflow: 'hidden', boxShadow: '0 28px 70px rgba(15,23,42,.28)' }}>
        <div style={{ padding: '22px 24px', borderBottom: '1px solid #EEF2F7', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          ...
          <button type="button" onClick={onClose} style={{ width: 34, height: 34, borderRadius: 8, border: 'none', background: '#F3F6FB', color: '#64748B', cursor: 'pointer', fontSize: 20, lineHeight: 1 }}>×</button>
        </div>
  ...
  return (
    ...
    <div className="cd-person-card" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px 10px' }}>
        ...
        <button className="voevoda-view-all" onClick={onOpenAll} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 13px', background: hovered ? '#EBF1FF' : '#F4F6FA', border: `1.5px solid ${hovered ? '#C7D2FE' : '#E5E7EB'}`, borderRadius: 20, fontSize: 12, fontWeight: 600, color: hovered ? '#375DFB' : '#6B7280', cursor: 'pointer', transition: 'all .25s ease' }}>
      ...
      <div style={{ display: 'flex', gap: 10, padding: '0 14px', marginBottom: 14 }}>
        ...
        <div style={{ width: badgeColW, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 7 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, flex: 1, alignContent: 'start' }}>
            ...
            {p.extraCount > 0 && (
              <div onClick={e => { e.stopPropagation(); navigate('/achievements'); }}
      ...
      <div style={{ padding: '0 16px', flex: 1, marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 19, fontWeight: 800, color: '#0D0F14', cursor: 'pointer', letterSpacing: '-.3px' }} onClick={() => onOpenModal(p.id)}>{p.name}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '0 14px 15px' }}>
        <button className="cd-btn-prim" onClick={() => navigate(personChatPath(p.id))} style={{ background: '#375DFB', border: 'none', borderRadius: 8, padding: '11px 0', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 14px rgba(55,93,251,.28)' }}>Связаться</button>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 22px', borderBottom: isLast ? 'none' : '1px solid #F5F5F7', background: hov ? '#FAFBFF' : 'transparent', transition: 'background .14s' }} onMouseOver={() => setHov(true)} onMouseOut={() => setHov(false)}>
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
        {p.smallImages.slice(0, 3).map((src, i) => <BadgeBox key={i} src={src} size={46} tooltip={BADGE_TOOLTIPS[i]} />)}
        {p.extraCount > 0 && (
          <div style={{ width: 46, height: 46, borderRadius: 12, background: '#EBF1FF', border: '1px solid #C7D2FE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#375DFB', cursor: 'pointer', transition: 'all .18s' }}
            ...
            onClick={() => navigate('/achievements')}>+{p.extraCount}</div>
        )}
      </div>
    </div>
  );
      ...
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, padding: 20, backdropFilter: 'blur(6px)', animation: 'fadeIn .2s ease' }}>
        <div onClick={e => e.stopPropagation()} style={{ maxWidth: 740, width: '100%', background: '#fff', borderRadius: 22, maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 28px 70px rgba(0,0,0,.22)', animation: 'scaleIn .22s cubic-bezier(.4,0,.2,1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 20px', borderBottom: '1px solid #F0F0F0', flexShrink: 0, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 8, padding: '6px 12px', flex: '0 1 180px' }}>
            ...
            <select value={cityFilter} onChange={e => setCityFilter(e.target.value)} style={{ padding: '6px 12px', border: '1px solid #E5E7EB', borderRadius: 8, fontSize: 13, color: '#374151', background: '#F9FAFB', cursor: 'pointer', outline: 'none' }}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ maxWidth: 700, width: '100%', background: '#fff', borderRadius: 22, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 28px 70px rgba(0,0,0,.22)', animation: 'scaleIn .22s cubic-bezier(.4,0,.2,1)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 24px', borderBottom: '1px solid #F0F0F0', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            {p.extraCount > 0 && <div style={{ width: 44, height: 44, borderRadius: 12, background: '#EBF1FF', border: '1px solid #C7D2FE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#375DFB', cursor: 'pointer', transition: 'all .2s' }} onClick={() => { onClose(); navigate('/achievements'); }}>+{p.extraCount}</div>}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, padding: '0 24px 22px' }}>
          <button className="cd-btn-prim" onClick={() => { onClose(); navigate(personChatPath(p.id)); }} style={{ background: '#375DFB', border: 'none', borderRadius: 8, padding: '13px 0', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 5px 18px rgba(55,93,251,.32)' }}>Связаться</button>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginTop: 14 }}>
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{ width: 34, height: 34, borderRadius: 8, border: '1px solid #E5E7EB', background: page === 0 ? '#F9FAFB' : '#fff', cursor: page === 0 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === 0 ? .4 : 1, transition: 'all .15s' }}>
                ...
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button key={i} onClick={() => setPage(i)} style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${i === page ? '#375DFB' : '#E5E7EB'}`, background: i === page ? '#375DFB' : '#fff', color: i === page ? '#fff' : '#374151', fontSize: 13, fontWeight: i === page ? 700 : 400, cursor: 'pointer', transition: 'all .15s' }}>{i + 1}</button>
                ))}
                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} style={{ width: 34, height: 34, borderRadius: 8, border: '1px solid #E5E7EB', background: page === totalPages - 1 ? '#F9FAFB' : '#fff', cursor: page === totalPages - 1 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: page === totalPages - 1 ? .4 : 1, transition: 'all .15s' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minHeight: '100%' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              ...
              <textarea value={myText} onChange={e => setMyText(e.target.value)} placeholder="Расскажите о курсе: что понравилось, что можно улучшить..." rows={4}
                style={{ width: '100%', boxSizing: 'border-box' as const, padding: '12px 14px', border: '1.5px solid #E5E7EB', borderRadius: 14, fontSize: 13, color: '#374151', resize: 'none', outline: 'none', lineHeight: 1.6, transition: 'border-color .15s', background: '#F9FAFB', fontFamily: 'inherit', flex: 1, minHeight: 140 }}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleSubmit} disabled={!myText.trim()} className="cd-btn-prim"
                style={{ flex: 1, padding: '12px 0', background: myText.trim() ? '#375DFB' : '#E5E7EB', border: 'none', borderRadius: 8, color: myText.trim() ? '#fff' : '#9CA3AF', fontSize: 14, fontWeight: 700, cursor: myText.trim() ? 'pointer' : 'default', boxShadow: myText.trim() ? '0 5px 18px rgba(55,93,251,.32)' : 'none', transition: 'all .2s' }}>
                Опубликовать отзыв
              </button>
              <button onClick={() => setTab('reviews')} style={{ padding: '12px 20px', background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, fontSize: 13, color: '#374151', cursor: 'pointer', transition: 'all .15s' }}
...
function FilterDropdown({ options, value, onChange }: { options: { key: string; label: string }[]; value: string; onChange: (k: string) => void }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#fff', border: `1px solid ${open || value !== options[0].key ? '#C7D2FE' : '#E5E7EB'}`, borderRadius: 8, fontSize: 13, color: open || value !== options[0].key ? '#375DFB' : '#374151', cursor: 'pointer', transition: 'all .15s' }}>
        ...
        {value !== options[0].key && current ? current.label : 'Фильтры'}
        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}><polyline points="6 9 12 15 18 9"/></svg>
      </button>
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, background: '#fff', border: '1px solid #E5E7EB', borderRadius: 10, boxShadow: '0 10px 30px rgba(0,0,0,.13)', zIndex: 60, minWidth: 190, overflow: 'hidden', animation: 'fadeIn .12s ease' }}>
          {options.map(o => (
            <button key={o.key} onClick={() => { onChange(o.key); setOpen(false); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '10px 14px', background: o.key === value ? '#F3F6FF' : 'transparent', border: 'none', fontSize: 13, color: o.key === value ? '#375DFB' : '#374151', fontWeight: o.key === value ? 600 : 400, cursor: 'pointer', textAlign: 'left' }}
              onMouseEnter={e => { if (o.key !== value) e.currentTarget.style.background = '#F9FAFB'; }} onMouseLeave={e => { if (o.key !== value) e.currentTarget.style.background = 'transparent'; }}>
              {o.label}
              {o.key === value && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#375DFB" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>}
            </button>
          ))}
        </div>
      )}
...
export function CourseDetailPage() {
  ...
  useEffect(() => { injectCss(CSS, 'cd-css-v5'); }, []);

  const displayedLessons = LESSONS.filter(l => {
    if (lessonFilter === 'available') return !l.locked;
    if (lessonFilter === 'locked') return l.locked;
    if (lessonFilter === 'viewed') return viewedLessons[String(l.id)]?.status === 'viewed';
    if (lessonFilter === 'unviewed') return !l.locked && viewedLessons[String(l.id)]?.status !== 'viewed';
    return true;
  });
  const displayedHomeworks = HOMEWORKS.filter(hw => {
    if (hwFilter === 'urgent') return hw.deadlineColor === '#F59E0B';
    if (hwFilter === 'normal') return hw.deadlineColor !== '#F59E0B';
    return true;
  });
  ...
  return (
    <div style={{ paddingTop: 60, marginLeft: 56, minHeight: '100vh', background: '#F4F6FB' }}>
      ...
      <div style={{ background:'#fff', borderBottom:'1px solid #E5E7EB', padding:'10px 28px' }}>
        <PortalBreadcrumb className="course-breadcrumb" items={[{ label:'Главная', to:'/' }, { label:'Мои курсы', to:'/my-courses' }, { label:courseName }]} />
      </div>
      ...
      <div style={{ padding: '24px 28px 56px' }}>
        ...
        <AnimSec style={{ marginBottom: 22 }}>
          <div className="cd-hero-banner" style={{ position: 'relative', borderRadius: 26, overflow: 'hidden', minHeight: 440, background: '#0a0a14', display: 'flex', alignItems: 'center' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(8,8,20,.97) 38%, rgba(8,8,20,.25) 100%)' }} />
            <div className="cd-hero-img" style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '46%' }}>
            ...
            <div style={{ position: 'relative', zIndex: 2, padding: '44px 48px', maxWidth: 640 }}>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', animation: 'fadeUp .5s ease .16s both' }}>
                <button className="cd-btn-prim" onClick={() => document.getElementById('lessons-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
                  style={{ padding: '14px 32px', background: '#375DFB', border: 'none', borderRadius: 8, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 22px rgba(55,93,251,.5)' }}>
                  К списку занятий
                </button>
                <button onClick={() => navigate('/messages?chat=1')}
                  style={{ padding: '14px 24px', background: 'rgba(255,255,255,.08)', border: '1px solid rgba(255,255,255,.2)', borderRadius: 8, color: '#fff', fontSize: 15, cursor: 'pointer', backdropFilter: 'blur(8px)', transition: 'background .2s, transform .12s' }}
                ...
                <button onClick={() => navigate(`/my-courses/${slug}/progress`, { state: { title: courseName } })}
                  style={{ padding: '14px 24px', background: 'rgba(55,93,251,.35)', border: '1px solid rgba(55,93,251,.6)', borderRadius: 8, color: '#fff', fontSize: 15, cursor: 'pointer', backdropFilter: 'blur(8px)', transition: 'background .2s, transform .12s', display: 'flex', alignItems: 'center', gap: 8 }}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            <div className="cd-meta-card" style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <button className="cd-btn-prim" onClick={() => navigate('/messages?chat=1')} style={{ padding: '9px 20px', background: '#EBF1FF', border: '1px solid #C7D2FE', borderRadius: 8, color: '#375DFB', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Задать вопрос по курсу</button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0, paddingLeft: 22, borderLeft: '1px solid #F0F0F0', alignSelf: 'stretch', justifyContent: 'center' }}>
            ...
            <div className="cd-meta-card" style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 28, flexWrap: 'wrap', flex: 1 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '24px 26px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22, flexWrap: 'wrap', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: '#9CA3AF', background: '#F4F6FA', padding: '3px 10px', borderRadius: 20, border: '1px solid #E5E7EB' }}>{displayedLessons.length} занятий</span>
              </div>
              <FilterDropdown value={lessonFilter} onChange={setLessonFilter} options={[{ key: 'all', label: 'Все занятия' }, { key: 'available', label: 'Доступные' }, { key: 'viewed', label: 'Просмотренные' }, { key: 'unviewed', label: 'Не просмотренные' }, { key: 'locked', label: 'Заблокированные' }]} />
            </div>
            <div className="cd-lesson-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
              {displayedLessons.length === 0 && <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px 0', color: '#9CA3AF', fontSize: 14 }}>Занятий по выбранному фильтру нет</div>}
              {displayedLessons.map((l, idx) => (
                <div key={l.id} className={`c-mil-shell cd-lesson-shell${l.locked ? ' locked' : ''}`}
                  onClick={() => { if (!l.locked) navigate(`/lessons/${l.id}`, { state: { courseTitle: courseName, courseSlug: slug } }); }}
                  style={{ animation: `fadeUp .45s ease ${idx * 65}ms backwards`, cursor: l.locked ? 'default' : 'pointer' }}>
                  <div className="c-card-wrap" data-locked={l.locked ? '' : undefined}
                    style={{ cursor: l.locked ? 'default' : 'pointer' }}>
                    <div className="c-card-img" style={{ position: 'relative', background: '#F3F4F6' }}>
                      ...
                      {!l.locked && viewedLessons[String(l.id)]?.status === 'viewed' && <div className="cd-viewed-badge" style={{ position:'absolute',top:10,right:10,zIndex:3,height:30,display:'flex',alignItems:'center',gap:6,background:'rgba(16,185,129,.94)',backdropFilter:'blur(6px)',color:'#fff',fontSize:11,fontWeight:800,padding:'0 9px',borderRadius:20,border:'1px solid rgba(255,255,255,.25)',boxSizing:'border-box' }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" style={{flexShrink:0}}><polyline points="20 6 9 17 4 12"/></svg><span className="cd-viewed-label">Просмотрено</span></div>}
                      ...
                      {!l.locked && viewedLessons[String(l.id)]?.status !== 'viewed' && <div style={{ position: 'absolute', bottom: 10, right: 10, zIndex: 3, width: 34, height: 34, borderRadius: '50%', background: 'rgba(55,93,251,.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 10px rgba(55,93,251,.4)' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg></div>}
                    </div>
                    ...
                    <div className="c-expand-inner">
                      <div style={{ padding: '0 16px 16px' }}>
                        {l.locked
                          ...
                          : <button className="c-oi c-oi-1 c-enroll-btn" onClick={e => { e.stopPropagation(); navigate(`/lessons/${l.id}`, { state: { courseTitle: courseName, courseSlug: slug } }); }}
                              style={{ width: '100%', height: 44, background: '#375DFB', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
                              Перейти к занятию
        ...
        <AnimSec style={{ marginBottom: 22 }}>
          <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 26px', borderBottom: '1px solid #F0F0F0' }}>
              ...
              <FilterDropdown value={hwFilter} onChange={setHwFilter} options={[{ key: 'all', label: 'Все задания' }, { key: 'urgent', label: 'Срочные' }, { key: 'normal', label: 'Без срочности' }]} />
            </div>
            <div style={{ padding: '0 26px' }}>
              {displayedHomeworks.length === 0 && <div style={{ textAlign: 'center', padding: '32px 0', color: '#9CA3AF', fontSize: 14 }}>Заданий по выбранному фильтру нет</div>}
              {displayedHomeworks.map((hw, idx) => (
                <div key={hw.id} className="cd-hw-row" style={{ display: 'flex', alignItems: 'flex-start', gap: 18, padding: '20px 0', borderBottom: idx < displayedHomeworks.length - 1 ? '1px solid #F5F5F7' : 'none', animation: `fadeUp .4s ease ${idx * 70}ms both` }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10, flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="cd-btn-sec" onClick={() => navigate('/messages?chat=1')} style={{ background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: '9px 16px', fontSize: 13, color: '#374151', cursor: 'pointer' }}>Задать вопрос</button>
          ...
          <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '24px 26px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
              {SERIES_COURSES.map((c, idx) => (
                <div key={c.id} className={`c-mil-shell cd-series-shell${c.status === 'locked' ? ' locked' : ''}`}
                  onClick={() => { if (c.status !== 'locked') navigate(`/my-courses/${encodeURIComponent(c.title)}`, { state: { title: c.title } }); }}
                  style={{ animation: `fadeUp .45s ease ${idx * 55}ms backwards`, cursor: c.status === 'locked' ? 'default' : 'pointer' }}>
                  <div className="c-card-wrap" data-locked={c.status === 'locked' ? '' : undefined} style={{ cursor: c.status === 'locked' ? 'default' : 'pointer' }}>
                  <div className="c-card-img" style={{ position: 'relative', background: '#F3F4F6' }}>
                    <img src={c.img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', filter: c.status === 'locked' ? 'brightness(.5) saturate(.7)' : 'none' }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,.5) 0%, transparent 55%)', pointerEvents: 'none' }} />
                    <div className="c-card-overlay" />
                    <div style={{ position: 'absolute', top: 10, left: 10, zIndex: 3, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(6px)', color: '#fff', fontSize: 12, fontWeight: 700, padding: '3px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,.15)' }}>{c.num}</div>
                    {c.status === 'done' && <div style={{ position: 'absolute', inset: 0, zIndex: 3, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><IcCheck /></div>}
                    {c.status === 'locked' && <div className="c-lock-icon" style={{ position: 'absolute', inset: 0, zIndex: 3, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><IcLock /></div>}
                    {c.status === 'active' && <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 3, background: '#375DFB', color: '#fff', fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 8 }}>В процессе</div>}
                  </div>
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); toggleFavorite({ id:c.id + 2000, kind:'course', title:c.title, city:c.city, duration:c.duration, price:c.price, format:'Оффлайн', image:c.img, available:c.status !== 'locked' }); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: hasFavorite(c.id + 2000, 'course') ? '#EF4444' : '#D1D5DB', padding: 2, transition: 'color .15s' }} onMouseEnter={e => (e.currentTarget.style.color = '#EF4444')} onMouseLeave={e => (e.currentTarget.style.color = hasFavorite(c.id + 2000, 'course') ? '#EF4444' : '#D1D5DB')}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill={hasFavorite(c.id + 2000, 'course') ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                      </button>
                    </div>
                  </div>
                  </div>
                    ...
                    <div className="c-expand-inner">
                      <div style={{ padding: '0 16px 16px' }}>
                        {c.status === 'locked'
                          ? <div className="c-oi c-oi-1" style={{ background: '#F4F6FA', border: '1px solid #E5E7EB', borderRadius: 10, padding: '10px 12px', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0, marginTop: 1 }}><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                              <div style={{ fontSize: 12, color: '#6B7280', lineHeight: 1.5 }}>Откроется после прохождения предыдущего курса серии</div>
                            </div>
                          : <button className="c-oi c-oi-1 c-enroll-btn" onClick={e => { e.stopPropagation(); navigate(`/my-courses/${encodeURIComponent(c.title)}`, { state: { title: c.title } }); }}
                              style={{ width: '100%', height: 44, background: '#375DFB', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
                              {c.status === 'done' ? 'Повторить курс' : 'Продолжить курс'}
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                            </button>}
                      </div>
                    </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ flex: 1 }}>
                  {CLASSMATES.map((m, idx) => (
                    <div key={m.id} className="cd-mate-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: idx < CLASSMATES.length - 1 ? '1px solid #F5F5F7' : 'none', animation: `fadeUp .35s ease ${idx * 55}ms both` }}>
                      <div onClick={() => navigate(userProfilePath(m.name))} style={{ width: 48, height: 48, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, background: '#F3F4F6', border: '2px solid #E5E7EB', cursor: 'pointer' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5, flexWrap: 'wrap' }}>
                          <span onClick={() => navigate(userProfilePath(m.name))} style={{ fontSize: 15, fontWeight: 700, color: '#111', cursor: 'pointer' }}>{m.name}</span>
                      ...
                      <button className="cd-btn-sec" onClick={() => navigate(`/messages?chat=${m.id % 3 === 0 ? 7 : m.id % 2 === 0 ? 5 : 2}`)}
                        style={{ padding: '8px 16px', background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, fontSize: 13, color: '#374151', cursor: 'pointer', flexShrink: 0 }}
                ...
                <div style={{ padding: '16px 20px', borderTop: '1px solid #F0F0F0', display: 'flex', gap: 10 }}>
                  <button onClick={() => navigate('/messages?chat=5')} style={{ flex: 1, padding: '11px 0', background: '#F0F4FF', border: '1.5px solid #C7D2FE', borderRadius: 8, color: '#375DFB', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, transition: 'background .15s' }} onMouseEnter={e => { e.currentTarget.style.background = '#E0E9FF'; }} onMouseLeave={e => { e.currentTarget.style.background = '#F0F4FF'; }}>
                  ...
                  <button className="cd-btn-prim" onClick={() => navigate('/messages?chat=4')} style={{ flex: 1, padding: '11px 0', background: '#375DFB', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, boxShadow: '0 4px 14px rgba(55,93,251,.28)' }}>
              ...
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '20px 22px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
                  ...
                  <button onClick={() => navigate(`/my-courses/${slug}/progress`, { state: { title: courseName } })}
                    style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 13px', background: '#EBF1FF', border: '1.5px solid #C7D2FE', borderRadius: 8, fontSize: 12, fontWeight: 600, color: '#375DFB', cursor: 'pointer', transition: 'all .2s' }}
