...
export function MyCourses() {
  ...
  const progressRows = PROGRESS.map(p => p.id === 1
    ? {
        ...p,
        viewedLessons: `${viewedCount} / ${totalLessons}`,
        courseProgress: Math.round((viewedCount / totalLessons) * 100),
      }
    : p);
            ...
            action={
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontWeight: 600, color: '#374151', cursor: 'pointer' }}
                >
