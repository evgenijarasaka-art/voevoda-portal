...
import { PROFESSIONAL_COURSES, type Course } from '../data/courses';
...
if (typeof document !== 'undefined' && !document.getElementById('prof-courses-styles')) {
  ...
  s.textContent = `
    ...
    .prof-card-shell:hover .c-card-overlay { opacity:1; }
    .pc-text-wrap { overflow:hidden; max-width:0; transition:max-width .52s cubic-bezier(.4,0,.2,1); border-radius:0 8px 8px 0; }
    .prof-card-shell:hover .pc-text-wrap { max-width:240px; }
    .pc-series-text { white-space:nowrap; display:flex; align-items:center; opacity:0; transform:translateX(-10px); transition:opacity .38s .12s ease, transform .52s .08s cubic-bezier(.22,1,.36,1); }
    .prof-card-shell:hover .pc-series-text { opacity:1; transform:translateX(0); }
    .prof-card-shell:hover .c-lock-icon { animation:lockWiggle .7s .3s cubic-bezier(.36,.07,.19,.97) both, lockGlow .7s .3s ease both; }
    ...
    @media (prefers-reduced-motion:reduce) {
      .prof-card-shell, .c-card-img img, .c-card-overlay, .pc-series-text { transition:none !important; }
    }
...
function ProfCourseCard({ c, idx, isFav, onToggleFav, onAddToCart, alreadyInCart }: {
  ...
  const goCheckout = (e: React.MouseEvent) => {
    ...
    setTimeout(() => {
      navigate('/checkout', { state: { directItem: { id:c.id, kind:'course', title:c.title, city:c.city, duration:c.duration, price:c.newPrice, oldPrice:c.oldPrice, format:c.format || 'Онлайн', image:c.image, stream:'', isFav:false, isSelected:true } } });
  ...
  const BadgePill = () => c.seriesIndex !== undefined ? (
    <div style={{
      ...
      {c.seriesName && (
        <div className="pc-text-wrap">
          <div className="pc-series-text" style={{
  ...
  const ImgSection = () => (
    <div className="c-card-img" style={{ height: 190, overflow: 'hidden', position: 'relative', borderRadius: '16px 16px 0 0' }}>
      ...
      <div style={{ position: 'absolute', top: 12, right: 12, display: 'flex', gap: 6 }}>
        ...
        <div title={c.format || 'Онлайн'} style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(255,255,255,.95)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(0,0,0,.1)', backdropFilter: 'blur(4px)' }}>
  ...
  return (
    <div className="prof-card-shell">
      ...
      <div className="c-card-wrap" data-locked={c.locked ? '' : undefined}
           ...
           onClick={() => { if (!c.locked) navigate(`/professional/${encodeURIComponent(c.title)}`); }}>
        <ImgSection />
...
export function ProfessionalCourses() {
  ...
  const courses = useAdminCourses(PROFESSIONAL_COURSES, 'professional');
  ...
  let displayed: Course[] = activeTab === 'series' ? courses.filter(c => c.seriesIndex !== undefined) : courses;
  ...
  const handleToggleFav = (e: React.MouseEvent, c: Course) => {
    e.stopPropagation();
    toggleFav({ id: c.id, kind: 'course', title: c.title, city: c.city, duration: c.duration, price: c.newPrice, format: c.format || 'Онлайн', image: c.image || '/проф1.png' });
  };
  const handleAddToCart = (c: Course) => {
    addCourse({ id: c.id, kind: 'course', title: c.title, city: c.city, duration: c.duration, price: c.newPrice, format: c.format || 'Онлайн', image: c.image || '/проф1.png', stream: '' });
  };
      ...
      <section style={{ padding: isMobile ? '16px 16px 0' : '24px 24px 0' }}>
        <div style={{ background: '#fff', borderRadius: 20, padding: isMobile ? '20px' : '24px 28px', border: '1px solid #E5E7EB', animation: 'cCardIn .5s ease' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <div style={{ position: 'relative' }} ref={dropRef}>
                <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #E5E7EB', borderRadius: 8, background: '#F9FAFB', overflow: 'hidden' }}>
                  <button onClick={() => { const i = PERIODS.indexOf(period); const prev = PERIODS[(i - 1 + PERIODS.length) % PERIODS.length]; if (prev === 'Календарь') { setShowCal(true); } else { setPeriod(prev); setShowDrop(false); setShowCal(false); } }} style={{ padding: '7px 8px', background: 'none', border: 'none', cursor: 'pointer', color: '#6B7280', fontSize: 16, fontWeight: 700, lineHeight: 1 }}>&lsaquo;</button>
                  ...
                  <button onClick={() => { const i = PERIODS.indexOf(period); const next = PERIODS[(i + 1) % PERIODS.length]; if (next === 'Календарь') { setShowCal(true); } else { setPeriod(next); setShowDrop(false); setShowCal(false); } }} style={{ padding: '7px 8px', background: 'none', border: 'none', cursor: 'pointer', color: '#6B7280', fontSize: 16, fontWeight: 700, lineHeight: 1 }}>&rsaquo;</button>
                </div>
                {showDrop && !showCal && (
          ...
          <div style={{ position: 'relative', overflow: 'visible' }}>
            ...
            {displayed.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 0 32px', color: '#9CA3AF' }}>
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" style={{ marginBottom: 14, opacity: .4 }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#6B7280', marginBottom: 6 }}>Курсы не найдены</div>
