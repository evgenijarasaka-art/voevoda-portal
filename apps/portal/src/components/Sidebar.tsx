...
// URL админ-панели (apps/admin). Приоритет: env-переменная → локальный vite на
// :3001, если портал открыт локально → продакшен-админка на Vercel. Без этого
// фолбэка кнопка на проде вела на localhost и «не работала» у заказчика.
const ADMIN_URL = (import.meta.env.VITE_ADMIN_URL as string)
  || (['localhost', '127.0.0.1'].includes(window.location.hostname)
    ? 'http://localhost:3001/'
    : 'https://admin-indol-tau-62.vercel.app/');

export const SIDEBAR_ICONS: Record<string, React.ReactNode> = {
  admin: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10Z"/><path d="m9 12 2 2 4-4"/></svg>,
...
// Позиция прокрутки списка сайдбара. Живёт на уровне модуля, а не в состоянии
// компонента, чтобы переживать любые перерисовки/ремаунты при навигации —
// иначе при переходах список «кидает наверх».
...
export function Sidebar() {
  ...
  const go = (path: string) => {
    navigate(path);
    // Не схлопываем сайдбар под курсором: пока мышь на нём — он остаётся
    // раскрытым (можно кликать страницы подряд), свернётся сам по mouseleave.
    // На тач-устройствах mouseleave не придёт, поэтому там сворачиваем по клику.
    if (window.matchMedia?.('(hover: none)').matches) closeSidebar();
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ width: expanded ? 240 : 56, background: '#fff', borderRight: subOpen ? 'none' : '1px solid #E5E7EB', display: 'flex', flexDirection: 'column', transition: 'width .22s cubic-bezier(.4,0,.2,1)', overflow: 'hidden', height: '100%' }}>
        <div
          ref={el => { if (el && el.scrollTop !== savedSidebarScroll) el.scrollTop = savedSidebarScroll; }}
        ...
        >
          ...
          {/* «Админ панель» — сразу после «Реклама», только для админов (роль admin/super).
              Нативная ссылка, а не window.open: встроенные браузеры (Telegram и т.п.)
              блокируют программное открытие окон — у заказчика кнопка «не работала». */}
          {isAdmin && (
            <a
              key="admin-panel"
              href={ADMIN_URL}
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '6px 8px', padding: '10px 10px', cursor: 'pointer', whiteSpace: 'nowrap', background: '#0A0A0A', color: '#fff', borderRadius: 10, transition: 'background .15s', textDecoration: 'none' }}
              onMouseOver={e => (e.currentTarget.style.background = '#222')}
              onMouseOut={e => (e.currentTarget.style.background = '#0A0A0A')}>
              <span style={{ flexShrink: 0, color: '#fff' }}>{SIDEBAR_ICONS.admin}</span>
              {expanded && (
                <>
                  <span style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>Админ панель</span>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ opacity: .7 }}><path d="M7 17 17 7M7 7h10v10" /></svg>
                </>
              )}
            </a>
          )}
      ...
      {expanded && subOpen && (
        <div
          ...
          <div className="sidebar-no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '12px 0' }}>
