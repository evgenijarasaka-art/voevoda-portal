...
import { useAdminTraining, getCourseLandingFor } from '../data/adminSiteData';
...
const PROGRAM_IMAGES = ['/voendelo1.png', '/voendelo2.png', '/voendelo3.png', '/voendelo4.png', '/voendelo5.png', '/voendelo6.png', '/оружие1.png', '/оружие2.png', '/медицина1.png'];
...
export function CourseLandingPage() {
  ...
  const adminLanding = getCourseLandingFor(adminTraining, courseName);
  const heroTitle = adminLanding?.heroTitle || courseName;
  const heroSubtitle = adminLanding?.heroSubtitle || 'Практический курс военной подготовки для тех, кто хочет уверенно действовать в группе, понимать полевую тактику и безопасно работать с учебным оружием.';
  const heroBadge = adminLanding?.heroBadge || 'военная подготовка';
  const heroImage = adminLanding?.heroImage || '/СолдатКурса.png';
  const startText = adminLanding?.startText || 'Ближайший старт группы';
  const benefits = adminLanding?.benefits?.length ? adminLanding.benefits : BENEFITS;
  const requirements = adminLanding?.requirements?.length ? adminLanding.requirements : REQUIREMENTS;
  const equipment = adminLanding?.equipment?.length ? adminLanding.equipment : EQUIPMENT;
  const photos = adminLanding?.photos?.length ? adminLanding.photos : PHOTOS;
  const programIntro = adminLanding?.programIntro || 'На курсах военной подготовки в СКВП мы делаем акцент на подготовку бойца, способного действовать в составе ВС РФ против регулярной армии. Это важное уточнение, поскольку на многих других курсах частично готовят бойцов, способных выполнять некоторые задачи полицейского спецназа против иррегулярных подразделений.';
  const programGroups = adminLanding?.programGroups?.length ? adminLanding.programGroups : null;
  const resultsText = adminLanding?.resultsText || 'На курсах военной подготовки в СКВП мы делаем акцент на подготовку бойца, способного действовать в составе ВС РФ против регулярной армии. Это важное уточнение, поскольку на многих других курсах частично готовят бойцов, способных выполнять некоторые задачи полицейского спецназа против иррегулярных подразделений.';
  const mapTitle = adminLanding?.mapTitle || 'Где проходит курс';
  const mapSubtitle = adminLanding?.mapSubtitle || 'Полигон «Калибр» · Минское шоссе, 31-й километр, с1, Москва';
  const reviewsTitle = adminLanding?.reviewsTitle || 'Отзывы о курсе';
  const coursePrice = Number(adminLanding?.price) || (courseName.toLowerCase().includes('молодого бойца') ? 35000 : 29000);
  const videoTitle = adminLanding?.videoTitle || 'Видео-презентация';
  const videoUrl = adminLanding?.videoUrl || '/video/den-rossii.mp4';
  const videoPoster = adminLanding?.videoPoster || '/video/den-rossii.jpg';
  const instructorName = adminLanding?.instructorName || 'Торнадо';
  const instructorRank = adminLanding?.instructorRank || 'Вице-ст. сержант';
  const instructorRole = adminLanding?.instructorRole || 'Главный инструктор';
  const instructorImage = adminLanding?.instructorImage || '/сержант.png';
  const instructorExp = adminLanding?.instructorExp || '12 лет';
  const instructorTrained = adminLanding?.instructorTrained || '480';
  const instructorCourses = adminLanding?.instructorCourses || '8';
  const studentsCount = adminLanding?.studentsCount || '14 888';
  const ratingValue = adminLanding?.rating || '5.0';
  const reviewsCount = adminLanding?.reviewsCount || '192';
  const seatsFree = adminLanding?.seatsFree ?? 8;
  const seatsTotal = adminLanding?.seatsTotal ?? 20;
            ...
            <Sec style={{ marginBottom: 22 }}>
              <div className="cl-hero-card" style={{ position: 'relative', borderRadius: 26, overflow: 'hidden', minHeight: 500, background: '#0a0a14', display: 'flex', alignItems: 'center' }}>
                ...
                <div className="cl-hero-media" style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: '58%' }}>
                  {!imgErrs['hero']
                    ? <img className="cl-hero-image" src={heroImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top', display: 'block' }} onError={() => setE('hero')} />
                ...
                <div style={{ position: 'relative', zIndex: 2, padding: '44px 52px 44px', width: '52%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 500 }}>
                  <div>
                    ...
                    <div className="cl-hero-badge" style={{ display: 'inline-flex', alignItems: 'center', background: 'rgba(255,255,255,.1)', border: '1px solid rgba(255,255,255,.25)', borderRadius: 999, padding: '5px 12px', fontSize: 13, color: '#fff', marginBottom: 28, width: 'fit-content', backdropFilter: 'blur(8px)' }}>
                      ...
                      Курс
                      <span className="cl-hero-badge-extra">{heroBadge}</span>
                    </div>
                    {/* Title */}
                ...
                <div style={{ position: 'absolute', bottom: 28, right: 28, zIndex: 3, display: 'flex', gap: 12, animation: 'fadeUp .5s ease .22s both' }}>
                  <div className="cl-stat-chip" style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 14, padding: '12px 18px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: '#fff' }}>{studentsCount} бойцов</div>
                  ...
                  <div className="cl-stat-chip" style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 14, padding: '12px 18px' }}>
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 800, color: '#fff' }}>{ratingValue} рейтинг</div>
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,.6)' }}>на основе <button type="button" onClick={() => document.getElementById('section-reviews')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} style={{ padding: 0, border: 0, background: 'none', color: 'inherit', textDecoration: 'underline', cursor: 'pointer', font: 'inherit' }}>{reviewsCount} отзывов</button></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.05fr .95fr', gap: 20, alignItems: 'stretch' }}>
              <div style={{ background: 'linear-gradient(145deg,#fff 0%,#F8FAFF 100%)', borderRadius: 22, border: '1px solid #E5E7EB', padding: '18px 24px 20px', display: 'grid', gridTemplateColumns: '1fr auto', gridTemplateRows: 'auto 1fr auto', columnGap: 22, rowGap: 14, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ gridColumn: 1, display: 'flex', alignItems: 'center', gap: 16, minWidth: 0 }}>
                  <div style={{ width: 88, height: 88, borderRadius: 18, overflow: 'hidden', flexShrink: 0, background: '#F3F4F6', border: '3px solid #fff', boxShadow: '0 0 0 1px #DCE3F4,0 10px 24px rgba(34,62,128,.12)' }}>
                    {!imgErrs['instr'] ? <img src={instructorImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setE('instr')} /> : <div style={{ width: '100%', height: '100%', background: '#EBF1FF' }} />}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 11, color: '#9CA3AF', marginBottom: 3, textTransform: 'uppercase' as const, letterSpacing: '.5px' }}>{instructorRank}</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 20, fontWeight: 800, color: '#111' }}>{instructorName}</span>
                      <IVDisplay index={2463} rating={5.0} />
                    </div>
                    <div style={{ fontSize: 13, color: '#6B7280' }}>{instructorRole}</div>
                  </div>
                </div>
                ...
                <div style={{ gridColumn: 2, gridRow: '1 / 4', display: 'flex', flexDirection: 'column', gap: 14, flexShrink: 0, paddingLeft: 22, borderLeft: '1px solid #E8ECF4', justifyContent: 'space-between', minWidth: 138 }}>
                  {([['Опыт', instructorExp], ['Бойцов обучено', instructorTrained], ['Курсов проведено', instructorCourses]] as [string, string][]).map(([label, val]) => (
                    <div key={label}>
              ...
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', flexDirection: 'column', justifyContent:'center' }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: '#111', marginBottom: 16 }}>
                  {startText} — <span style={{ color: '#375DFB', fontWeight: 800 }}>{targetLabel}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap' }}>
                  <Countdown target={targetDate} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 9, minWidth: 168, flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#6B7280' }}><span>Свободно мест</span><span style={{ fontWeight: 800, color: '#111' }}>{seatsFree} / {seatsTotal}</span></div>
                    <div style={{ height: 8, background: '#F3F4F6', borderRadius: 99, overflow: 'hidden' }}><div style={{ width: `${Math.min(100, Math.max(0, Math.round(((Number(seatsTotal) - Number(seatsFree)) / Math.max(1, Number(seatsTotal))) * 100)))}%`, height: '100%', background: 'linear-gradient(90deg,#375DFB,#7B9FFF)', borderRadius: 99 }} /></div>
          ...
          <div id="section-video">
            <Sec style={{ marginBottom: 22 }}>
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '28px 32px' }}>
                <h2 style={{ fontSize: 22, fontWeight: 800, color: '#111', margin: '0 0 20px' }}>{videoTitle}</h2>
                <VoevodaPlayer src={videoUrl} poster={videoPoster} height={460} />
              </div>
            </Sec>
          </div>
          ...
          <div id="section-program">
            <Sec style={{ marginBottom: 22 }}>
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '28px 32px' }}>
                ...
                {programGroups ? programGroups.map((group, groupIndex) => (
                  <div key={group.id ?? group.title ?? groupIndex} style={{ marginBottom: groupIndex < programGroups.length - 1 ? 32 : 0 }}>
                    <h3 style={{ fontSize: 18, fontWeight: 700, color: '#111', margin: '0 0 18px' }}>{group.title}</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 18 }}>
                      {(group.items || []).map((title, itemIndex) => (
                        <DiscCard key={`${groupIndex}-${itemIndex}`} img={PROGRAM_IMAGES[(groupIndex * 3 + itemIndex) % PROGRAM_IMAGES.length]} title={title} text="Тема программы курса, управляется через админку." idx={itemIndex} prefix={`admin-${groupIndex}`} />
                      ))}
                    </div>
                  </div>
                )) : <>
          ...
          <div id="section-schedule">
            <Sec style={{ marginBottom: 22 }}>
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', padding: '28px 28px 24px' }}>
                ...
                <StreamCalendar courseName={courseName} />
              </div>
            </Sec>
          </div>
            ...
            <Sec style={{ marginBottom: 22 }}>
              <div style={{ background: '#fff', borderRadius: 22, border: '1px solid #E5E7EB', overflow: 'hidden' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid #F0F0F0' }}>
                  <h2 style={{ fontSize: 20, fontWeight: 700, color: '#111', margin: 0 }}>{mapTitle}</h2>
                  <div style={{ fontSize: 13, color: '#6B7280', marginTop: 4 }}>{mapSubtitle}</div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent:'space-between', gap: 18, marginBottom: 24, paddingBottom: 18, borderBottom: '1px solid #E8ECF4', flexWrap:'wrap' }}>
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      <div><h2 style={{ fontSize: 20, fontWeight: 800, color: '#111', margin: 0 }}>{reviewsTitle}</h2><div style={{fontSize:12,color:'#8A96AE',marginTop:3}}>Подтверждённые участники потока</div></div>
                    </div>
                  </div>
