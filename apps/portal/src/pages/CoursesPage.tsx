...
export function CoursesPage() {
  ...
  const adminCourses = useAdminCourses(MILITARY_COURSES, 'military');
  ...
  const cityCards = useMemo(() => {
    ...
    COURSES.forEach(course => counts.set(course.city, (counts.get(course.city) ?? 0) + 1));
    const known = new Set(CITIES.map(city => city.name));
    const synced = CITIES.map(city => ({ ...city, count: counts.get(city.name) ?? city.count }));
    const extra = [...counts.entries()]
      .filter(([name]) => !known.has(name))
      .map(([name, count], index) => ({ id: 100 + index, name, count, img: '/gorod1.png' }));
    return [...synced, ...extra];
  }, [COURSES]);
  const sortedCities = [...cityCards].sort((a, b) => citySort === 'desc' ? b.count - a.count : a.count - b.count);
