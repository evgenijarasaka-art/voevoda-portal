    ...
    <>
      ...
      {isHome ? children : (
        <div className="portal-app-main">
          <GlobalRouteHeader />
          {/* key=pathname перезапускает анимацию появления (routeIn) при каждом переходе */}
          <div className="portal-route-body" data-route={location.pathname} key={location.pathname}>{children}</div>
        </div>
      )}
    </>
  );
